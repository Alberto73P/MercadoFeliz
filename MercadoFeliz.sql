...
--
-- Base de datos: `mercadofeliz`
--
DROP DATABASE IF EXISTS `mercadofeliz`;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InsCliente` (IN `_RFC` VARCHAR(13), IN `_nombre` VARCHAR(20), IN `_apPat` VARCHAR(20), IN `_apMat` VARCHAR(20), IN `_tel` NUMERIC(15))   begin
    ...
    if not exists(select* from cliente where RFC=_RFC)then
    	insert into cliente values(_RFC, _nombre, _apPat, _apPat,_tel);
         select '0' as com_estatus;
    else
       select '1' as com_estatus;
    end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InsEmpleado` (IN `_RFC` VARCHAR(13), IN `_nombre` VARCHAR(20), IN `_apPat` VARCHAR(20), IN `_apMat` VARCHAR(20), IN `_tel` NUMERIC(15))   begin
    -- 1er validacion _rfc NO exista
    if not exists(select * from empleado where RFC = _RFC)then
    	insert into empleado values(_RFC, _nombre, _apPat, _apMat,_tel);
         select '0' as com_estatus;
    else
       select '1' as com_estatus;
    end if;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_UpdEmpleado` (IN `_RFC` VARCHAR(13), IN `_nombre` VARCHAR(20), IN `_apPat` VARCHAR(20), IN `_apMat` VARCHAR(20), IN `_tel` NUMERIC(15))   begin
    -- 1er validacion _rfc exista
    if exists(select* from empleado where RFC=_RFC)then
    	UPDATE empleado 
        SET nombre = _nombre, apellido_paterno = _apPat, apellido_materno = _apMat, telefono = _tel
